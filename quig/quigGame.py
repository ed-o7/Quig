...
def doAction(num,value,hp):#this is triggered after a dialogue is finished, executed upon escape key entered
    ...
    if num == 1:#do nothing
        ...
        endGame = [True,value]
    elif num == 11:#giving demon cake to quig 
        (allCharacters[value[0]]).dia = allDialogues[value[1]]
        inventory.append("Belphegor Cake")
        (allCharacters[4]).dia = allDialogues[8]
        stage_y,stage_x = 5,3
        cakes += 1
    elif num == 12:#adds something to quig's inventory and changes
        (allCharacters[value[0]]).dia = allDialogues[value[1]]
        ...
        trugDia1 = dialogue("Trug",
                           ...
                           [[True,"WeeweeWa2qa."],[True,"Huh?"],[True, "Where do I get leaves, dude?"],[True,"Nah, dude. Far out."]],
...
while active:
    ...
    if madmoder:
        print("Mad.")
        grass = (random.randint(0,255),random.randint(0,255),random.randint(0,255))
